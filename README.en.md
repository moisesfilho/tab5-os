...
- **Application Isolation & WebAssembly Runtime (WAMR)** — isolated sandboxed execution powered by WebAssembly Micro Runtime (WAMR), providing security, operating system stability, and decoupled process lifecycles
- **Host ABI & Storage Sandboxing** — native bindings exposing safe access to LVGL UI (App Bar, Virtual Keyboard, Toast Notifications), hardware (INA226 battery, Wi-Fi, BLE, speaker, clock, and logs) and private app storage directories (`/sdcard/data/<app_id>`)
- **Package Manager (`.tab5pkg`) & Integrated Installer** — standardized package format with `manifest.json`, dynamic app discovery and registration at boot, SD precedence over embedded firmware ROM, direct `.tab5pkg` association in Files app, and visual permission audit modal before installation
- **Storage & Memory Manager App** — 3-tab system application providing visual monitoring of Flash memory (apps partition), SD card, and RAM/PSRAM heap, with installed apps management, clean uninstall with sandbox data cleanup, and pending package detection
- **Tab5 App SDK & Build Toolchain** — complete development kit for building standalone C/C++ applications (`sdk/tab5-app-sdk`), including official headers (`tab5_sdk.h`), CLI packaging tool (`tools/pack.py`), CMake WebAssembly toolchain integration, templates, and working examples
