...
Valida o manifest.json e empacota uma aplicação em formato de arquivo único .tab5pkg
"""
...
def validate_manifest(manifest_path):
    if not os.path.exists(manifest_path):
        raise FileNotFoundError(
            f"Arquivo de manifesto nao encontrado: {manifest_path}")
    ...
    for field in ['id', 'name', 'version']:
        if field not in data or not str(data[field]).strip():
            raise ValueError(
                f"Campo obrigatorio ausente ou vazio no manifesto: '{field}'")

    app_id = str(data['id']).strip()
    if not VALID_ID_REGEX.match(app_id):
        raise ValueError(
            f"ID de aplicacao invalido '{app_id}'. Use apenas letras minusculas, numeros, pontos, hifens e underlines."
        )

    version = str(data['version']).strip()
    if not VALID_SEMVER_REGEX.match(version):
        raise ValueError(
            f"Versao '{version}' nao segue o padrao SemVer (ex: 1.0.0)")
...
def pack_app(app_dir, output_dir=None, package_name=None):
    ...
    if not os.path.exists(entry_file):
        raise FileNotFoundError(
            f"Binario de entrada '{entry_name}' nao encontrado em {app_dir}")
    ...
    dest_pkg_file = os.path.join(output_dir, package_name)
    if os.path.exists(dest_pkg_file):
        if os.path.isdir(dest_pkg_file):
            shutil.rmtree(dest_pkg_file)
        else:
    ...
    print(f"[OK] Pacote Tab5 criado com sucesso: {dest_pkg_file}")
    print(f"     App ID: {manifest_data['id']}")
    print(f"     Nome: {manifest_data['name']}")
    print(f"     Versao: {manifest_data['version']}")
    return dest_pkg_file


def main():
    parser = argparse.ArgumentParser(description="Tab5 OS Package Builder")
    parser.add_argument("app_dir",
                        help="Diretório da aplicação contendo manifest.json")
    parser.add_argument("-o",
                        "--output",
                        help="Diretório de saída do pacote",
                        default=None)
    parser.add_argument("-n",
                        "--name",
                        help="Nome customizado para o pacote .tab5pkg",
                        default=None)
